...
public class ObjectCreateService {
	...
	private ObjectCreateService() {
		throw new IllegalStateException("Utility class, can not be instantiated");
	}

	public static EnvironmentAttribute[] createObjects(ArrayList<Integer> warnErrorLimitsValueList,
			Map<String, String> environmentAttributesValues) {
		...
		for (String AttributeName : environmentAttributesValues.keySet()) {
			...
			objAttribute[flag1++] = new EnvironmentAttribute(AttributeName, currentVal,
					warnErrorLimitsValueList.get(flag2), warnErrorLimitsValueList.get(flag2 + 1),
					warnErrorLimitsValueList.get(flag2 + 2), warnErrorLimitsValueList.get(flag2 + 3), unit);
			flag2 += 4;
		}
